...
from DDQN_torch_test import DuelingDoubleDeepQNetwork
...
def monitor_reward(ue_RL_list, episode):
    ...
    for ue_RL in ue_RL_list:
        reward_store.append([reward for reward in ue_RL.reward_store[episode] if reward != 0])
    episode_sum_reward = [sum(reward_list) / len(reward_list) for reward_list in reward_store]
    # episode_sum_reward = sum(sum(ue_RL.reward_store[episode]) / len([reward for reward in ue_RL.reward_store[episode] if reward != 0]) for ue_RL in ue_RL_list)
    avg_episode_sum_reward = sum(episode_sum_reward) / len(episode_sum_reward)
    print(f"reward(4time_step): {avg_episode_sum_reward}")
...
def monitor_delay(ue_RL_list, episode):
    ...
    delay_store = []
    for ue_RL in ue_RL_list:
        delay_store.append([delay for delay in ue_RL.delay_store[episode] if delay != 0])
    episode_sum_delay = [sum(delay_list) / len(delay_list) for delay_list in delay_store]
    avg_delay_in_episode = sum(episode_sum_delay) / len(episode_sum_delay)
    print(f"delay(4time_step): {avg_delay_in_episode}")
...
def monitor_energy(ue_RL_list, episode):
    ...
    energy_store = []
    for ue_RL in ue_RL_list:
        energy_store.append([energy for energy in ue_RL.energy_store[episode] if energy != 0])
    episode_sum_energy = [sum(energy_list) / len(energy_list) for energy_list in energy_store]
    avg_energy_in_episode = sum(episode_sum_energy) / len(episode_sum_energy)
    print(f"energy(4time_step): {avg_energy_in_episode}")
        ...
        while True:
            ...
            if done:
                ...
                if episode % 10 == 0:
                    ...
                    # '''
                # if episode % (NUM_EPISODE-1) == 0 and episode != 0:
                if episode == 1999 or episode == 1499 or episode == 999 or episode == 499 or (episode % (NUM_EPISODE-1) == 0 and episode != 0):
                    fig, axs = plt.subplots(5, 1, figsize=(8, 16))
                    axs[0].plot(avg_reward_list, '-')
                    axs[0].set_ylabel('reward')
                    axs[1].plot(avg_delay_list_in_episode, '-')
                    axs[1].set_ylabel('dalay')
                    axs[2].plot(avg_energy_list_in_episode, '-')
                    axs[2].set_ylabel('energy')
                    axs[3].plot(num_task_drop_list_in_episode, '-')
                    axs[3].set_ylabel('drop_num')
                    axs[4].plot(ue_RL_list[0].loss_store, '-')
                    axs[4].set_ylabel('loss')
                    plt.savefig('figures_test_' + str(episode+1) + str(env.task_arrive_prob) + '.png')
                    ...
                    with open('record.csv', mode='r') as file:
                        ...
                        for row in reader:
                            data.append(row)
                    data.append([env.n_ue, env.n_edge, episode+1, env.task_arrive_prob, 'QOCO',
                                    avg_reward_list[-1], avg_delay_list_in_episode[-1],
                                   avg_energy_list_in_episode[-1], num_task_drop_list_in_episode[-1]])
                    with open('record.csv', mode='w', newline='') as file:
                        writer3 = csv.writer(file)
                        writer3.writerows(data)


                    # import csv
                    # with open('record.csv', 'w', newline='') as csvfile:
                    #     fieldnames = ['USER', 'EDGE', 'EPISODE', 'TASK_ARRIVE_PROB', 'decision', 'reward', 'delay', 'energy', 'drop']
                    #     writer2 = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    #     writer2.writeheader()
                    #     writer2.writerow({'USER': env.n_ue,
                    #                      'EDGE': env.n_edge,
                    #                      'EPISODE': NUM_EPISODE,
                    #                      'TASK_ARRIVE_PROB': env.task_arrive_prob,
                    #                      'decision':'QOCO',
                    #                      'reward': avg_reward_list[-1],
                    #                      'delay': avg_delay_list_in_episode[-1],
                    #                      'energy': avg_energy_list_in_episode[-1],
                    #                      'drop': num_task_drop_list_in_episode[-1]})
...
def test(ue_RL_list, NUM_EPISODE):
    avg_reward_list = []
    avg_reward_list_2 = []
    avg_delay_list_in_episode = []
    avg_energy_list_in_episode = []
        ...
        while True:
            ...
            if done:
                ...
                if episode % 10 == 0:
                    ...
                    # '''
                # if episode % (NUM_EPISODE-1) == 0 and episode != 0:
                if episode == 1999 or episode == 999 or (episode % (NUM_EPISODE-1) == 0 and episode != 0):
                    fig, axs = plt.subplots(5, 1, figsize=(8, 16))
                    axs[0].plot(avg_reward_list, '-')
                    axs[0].set_ylabel('reward')
                    axs[1].plot(avg_delay_list_in_episode, '-')
                    ...
                    axs[3].plot(num_task_drop_list_in_episode, '-')
                    axs[3].set_ylabel('drop_num')
                    plt.savefig('figures_edge_' + str(episode+1) + '.png')
                    # '''
    ...
    if mode == 'DDQN':
        for ue in range(Config.N_UE):
            ue_RL_list.append(DuelingDoubleDeepQNetwork(env.n_actions, env.n_features, env.n_lstm_state, env.n_time,
                                                        ...
                                                        # each 200 steps, update target net
        ...
        for ue in range(Config.N_UE):
            ue_RL_list.append(LocalModel(env.n_time))

        test(ue_RL_list, Config.N_EPISODE)
    elif mode == 'EDGE':
        for ue in range(Config.N_UE):
            ue_RL_list.append(EdgeModel(env.n_time))
